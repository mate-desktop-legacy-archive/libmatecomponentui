 ...
 * a MateComponentDockItemGrip in addition to it's child,
...
struct _MateComponentDockItemPrivate
{
  GtkWidget *child;
  GtkWidget *grip;
...
G_DEFINE_TYPE (MateComponentDockItem, matecomponent_dock_item, GTK_TYPE_BIN)
...
static guint     get_preferred_width   (MateComponentDockItem *item);
static guint     get_preferred_height  (MateComponentDockItem *item);

static void matecomponent_dock_item_set_property   (GObject            *object,
					     guint               param_id,
					     const GValue       *value,
					     GParamSpec         *pspec);
static void matecomponent_dock_item_get_property   (GObject            *object,
					     guint               param_id,
					     GValue             *value,
					     GParamSpec         *pspec);
static void matecomponent_dock_item_finalize       (GObject           *object);
static void matecomponent_dock_item_map            (GtkWidget         *widget);
static void matecomponent_dock_item_unmap          (GtkWidget         *widget);
static void matecomponent_dock_item_realize        (GtkWidget         *widget);
static void matecomponent_dock_item_unrealize      (GtkWidget         *widget);
static void matecomponent_dock_item_style_set      (GtkWidget         *widget,
					     GtkStyle          *previous_style);
static void matecomponent_dock_item_size_request   (GtkWidget         *widget,
					     GtkRequisition    *requisition);
static void matecomponent_dock_item_size_allocate  (GtkWidget         *widget,
					     GtkAllocation     *real_allocation);
static void matecomponent_dock_item_add            (GtkContainer      *container,
					     GtkWidget         *widget);
static void matecomponent_dock_item_remove         (GtkContainer      *container,
					     GtkWidget         *widget);
static void matecomponent_dock_item_forall         (GtkContainer     *container,
					     gboolean          include_internals,
					     GtkCallback       callback,
					     gpointer          callback_data);
static void matecomponent_dock_item_paint          (GtkWidget         *widget,
					     GdkEventExpose    *event);
static gboolean matecomponent_dock_item_expose     (GtkWidget         *widget,
					     GdkEventExpose    *event);
static gboolean matecomponent_dock_item_button_changed (GtkWidget         *widget,
						 GdkEventButton    *event);
static gboolean matecomponent_dock_item_motion     (GtkWidget         *widget,
					     GdkEventMotion    *event);

static void matecomponent_dock_item_float_window_size_request (GtkWidget *widget, GtkRequisition *requisition, gpointer data);
static void matecomponent_dock_item_float_window_size_allocate (GtkWidget *widget, GtkAllocation *allocation, gpointer data);

static gboolean matecomponent_dock_item_float_window_expose (GtkWidget *widget, GdkEventExpose *event, gpointer data);
static gboolean matecomponent_dock_item_float_window_button_changed (GtkWidget *widget, GdkEventButton *event, gpointer data);
static gboolean matecomponent_dock_item_float_window_motion (GtkWidget *widget, GdkEventMotion *event, gpointer data);
...
static guint
get_preferred_width (MateComponentDockItem *dock_item)
{
  GtkWidget *child;
  ...
  if (dock_item->orientation == GTK_ORIENTATION_HORIZONTAL)
    preferred_width += MATECOMPONENT_DOCK_ITEM_NOT_LOCKED (dock_item) ? DRAG_HANDLE_SIZE : 0;
...
static guint
get_preferred_height (MateComponentDockItem *dock_item)
{
  GtkWidget *child;
  ...
  if (dock_item->orientation == GTK_ORIENTATION_VERTICAL)
    preferred_height += MATECOMPONENT_DOCK_ITEM_NOT_LOCKED (dock_item) ? DRAG_HANDLE_SIZE : 0;
...
static void
matecomponent_dock_item_class_init (MateComponentDockItemClass *klass)
{
  ...
  gobject_class->set_property = matecomponent_dock_item_set_property;
  gobject_class->get_property = matecomponent_dock_item_get_property;
  ...
  dock_item_signals[DOCK_DRAG_BEGIN] =
	  g_signal_new ("dock_drag_begin",
			...
			G_SIGNAL_RUN_LAST,
			G_STRUCT_OFFSET (MateComponentDockItemClass,
					 dock_drag_begin),
			NULL, NULL,
  ...
  dock_item_signals[DOCK_DRAG_MOTION] =
	  g_signal_new ("dock_drag_motion",
			G_TYPE_FROM_CLASS (gobject_class),
			G_SIGNAL_RUN_LAST,
			G_STRUCT_OFFSET (MateComponentDockItemClass, dock_drag_motion),
			NULL, NULL,
  ...
  dock_item_signals[DOCK_DRAG_END] =
	  g_signal_new ("dock_drag_end",
			G_TYPE_FROM_CLASS (gobject_class),
			G_SIGNAL_RUN_LAST,
			G_STRUCT_OFFSET (MateComponentDockItemClass, dock_drag_end),
			NULL, NULL,
			g_cclosure_marshal_VOID__VOID,
			G_TYPE_NONE, 0);
  ...
  dock_item_signals[DOCK_DETACH] =
	  g_signal_new ("dock_detach",
			G_TYPE_FROM_CLASS (gobject_class),
			G_SIGNAL_RUN_LAST,
			G_STRUCT_OFFSET (MateComponentDockItemClass, dock_detach),
			NULL, NULL,
			g_cclosure_marshal_VOID__VOID,
			G_TYPE_NONE, 0);
  ...
  dock_item_signals[ORIENTATION_CHANGED] =
	  g_signal_new ("orientation_changed",
			G_TYPE_FROM_CLASS (gobject_class),
			G_SIGNAL_RUN_LAST,
			G_STRUCT_OFFSET (MateComponentDockItemClass, orientation_changed),
			NULL, NULL,
			g_cclosure_marshal_VOID__ENUM,
			G_TYPE_NONE, 1, GTK_TYPE_ORIENTATION);

  gobject_class->finalize = matecomponent_dock_item_finalize;

  widget_class->map = matecomponent_dock_item_map;
  widget_class->unmap = matecomponent_dock_item_unmap;
  widget_class->realize = matecomponent_dock_item_realize;
  widget_class->unrealize = matecomponent_dock_item_unrealize;
  widget_class->style_set = matecomponent_dock_item_style_set;
  widget_class->size_request = matecomponent_dock_item_size_request;
  widget_class->size_allocate = matecomponent_dock_item_size_allocate;
  widget_class->expose_event = matecomponent_dock_item_expose;
  widget_class->button_press_event = matecomponent_dock_item_button_changed;
  widget_class->button_release_event = matecomponent_dock_item_button_changed;
  widget_class->motion_notify_event = matecomponent_dock_item_motion;

  container_class->add = matecomponent_dock_item_add;
  container_class->remove = matecomponent_dock_item_remove;
  container_class->forall = matecomponent_dock_item_forall;
}

static void
matecomponent_dock_item_init (MateComponentDockItem *dock_item)
{
  GTK_WIDGET_UNSET_FLAGS (dock_item, GTK_NO_WINDOW);

  dock_item->_priv = g_new0 (MateComponentDockItemPrivate, 1);

  dock_item->_priv->grip = matecomponent_dock_item_grip_new (dock_item);
  ...
  dock_item->behavior = MATECOMPONENT_DOCK_ITEM_BEH_NORMAL;
...
static void
matecomponent_dock_item_set_property (GObject            *object,
			      guint               param_id,
			      const GValue       *value,
			      GParamSpec         *pspec)
{
  ...
  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (object));

  dock_item = MATECOMPONENT_DOCK_ITEM (object);

  switch (param_id)
    {
    case PROP_SHADOW:
      matecomponent_dock_item_set_shadow_type (dock_item, g_value_get_enum (value));
      break;
    case PROP_ORIENTATION:
      matecomponent_dock_item_set_orientation (dock_item, g_value_get_enum (value));
      break;
    default:
...
static void
matecomponent_dock_item_get_property (GObject            *object,
			      guint               param_id,
			      GValue             *value,
			      GParamSpec         *pspec)
{
  ...
  switch (param_id)
    {
    case PROP_SHADOW:
      g_value_set_enum (value, matecomponent_dock_item_get_shadow_type (dock_item));
      break;
    case PROP_ORIENTATION:
      g_value_set_enum (value, matecomponent_dock_item_get_orientation (dock_item));
      break;
...
static void
matecomponent_dock_item_finalize (GObject *object)
{
  MateComponentDockItem *di;

  g_return_if_fail (object != NULL);
  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (object));

  di = MATECOMPONENT_DOCK_ITEM (object);
  ...
  G_OBJECT_CLASS (matecomponent_dock_item_parent_class)->finalize (object);
}

static void
matecomponent_dock_item_map (GtkWidget *widget)
{
  GtkBin *bin;
  MateComponentDockItem *di;

  g_return_if_fail (widget != NULL);
  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (widget));

  GTK_WIDGET_SET_FLAGS (widget, GTK_MAPPED);

  bin = GTK_BIN (widget);
  di = MATECOMPONENT_DOCK_ITEM (widget);
  ...
  if (di->is_floating && !di->float_window_mapped)
    matecomponent_dock_item_detach (di, di->float_x, di->float_y);
...
static void
matecomponent_dock_item_unmap (GtkWidget *widget)
{
  MateComponentDockItem *di;
...
static void
matecomponent_dock_item_realize (GtkWidget *widget)
{
  ...
  g_signal_connect (di->_priv->float_window, "size_allocate",
		    G_CALLBACK (matecomponent_dock_item_float_window_size_allocate),
		    di);

  g_signal_connect (di->_priv->float_window, "size_request",
		    G_CALLBACK (matecomponent_dock_item_float_window_size_request),
		    di);
  g_signal_connect (di->_priv->float_window, "expose_event",
		    G_CALLBACK (matecomponent_dock_item_float_window_expose),
		    di);

  g_signal_connect (di->_priv->float_window, "button_press_event",
		    G_CALLBACK (matecomponent_dock_item_float_window_button_changed),
		    di);
  ...
  g_signal_connect (di->_priv->float_window, "motion_notify_event",
		    G_CALLBACK (matecomponent_dock_item_float_window_motion),
		    di);
...
static void
matecomponent_dock_item_unrealize (GtkWidget *widget)
{
  MateComponentDockItem *di;
  ...
  if (di->float_window_mapped)
    matecomponent_dock_item_unfloat (di);

  gtk_widget_destroy (GTK_WIDGET (di->_priv->float_window));
  di->_priv->float_window = NULL;

  GTK_WIDGET_CLASS (matecomponent_dock_item_parent_class)->unrealize (widget);
}

static void
matecomponent_dock_item_style_set (GtkWidget *widget,
                           GtkStyle  *previous_style)
{
  MateComponentDockItem *di;
	      ...
	      MateComponentDockItem *dock_item)
{
  ...
  if (dock_item->orientation == GTK_ORIENTATION_HORIZONTAL)
    {
      requisition->width =
        MATECOMPONENT_DOCK_ITEM_NOT_LOCKED (dock_item) ? DRAG_HANDLE_SIZE : 0;
      if (bin->child != NULL)
        {
...
static void
matecomponent_dock_item_size_request (GtkWidget *widget,
			       GtkRequisition *requisition)
{
  ...
  dock_item = MATECOMPONENT_DOCK_ITEM (widget);
...
static void
matecomponent_dock_item_float_window_size_request (GtkWidget *widget,
					    GtkRequisition *requisition,
					    gpointer data)
{
  MateComponentDockItem *dock_item;
  ...
  dock_item = MATECOMPONENT_DOCK_ITEM (data);
...
static void
matecomponent_dock_item_float_window_size_allocate (GtkWidget *widget,
					     GtkAllocation *allocation,
					     gpointer data)
{
  GtkBin *bin;
  ...
  di = MATECOMPONENT_DOCK_ITEM (data);
  ...
  if (MATECOMPONENT_DOCK_ITEM_NOT_LOCKED(di))
...
static void
matecomponent_dock_item_size_allocate (GtkWidget     *widget,
				GtkAllocation *allocation)
{
  GtkBin *bin;
  MateComponentDockItem *di;
...
static void
matecomponent_dock_item_float_window_paint (GtkWidget *widget,
				     GdkEventExpose *event,
				     gpointer data)
{
  MateComponentDockItem *di;
...
static void
matecomponent_dock_item_paint (GtkWidget      *widget,
			GdkEventExpose *event)
{
  MateComponentDockItem *di;
...
static gboolean
matecomponent_dock_item_float_window_expose (GtkWidget *widget,
				      GdkEventExpose *event,
				      gpointer data)
{
  ...
  if (GTK_WIDGET_DRAWABLE (widget))
    {
      matecomponent_dock_item_float_window_paint (widget, event, data);

      GTK_WIDGET_CLASS (matecomponent_dock_item_parent_class)->expose_event (widget, event);
    }
...
static gboolean
matecomponent_dock_item_expose (GtkWidget      *widget,
			 GdkEventExpose *event)
{
  g_return_val_if_fail (widget != NULL, FALSE);
  g_return_val_if_fail (MATECOMPONENT_IS_DOCK_ITEM (widget), FALSE);
  g_return_val_if_fail (event != NULL, FALSE);

  if (GTK_WIDGET_DRAWABLE (widget) && event->window != widget->window)
    {
      matecomponent_dock_item_paint (widget, event);
...
static void
matecomponent_dock_item_drag_end (MateComponentDockItem *di)
{
  ...
  if (event->button == 1 && event->type == GDK_BUTTON_PRESS)
    {
      ...
      if (!di->is_floating)
        ...
        child = GTK_WIDGET (matecomponent_dock_item_get_child (di));
      ...
      if (in_handle)
	{
	  ...
	  di->dragoff_y = event->y;
          ...
          matecomponent_dock_item_grab_pointer (di);
    ...
    {
      matecomponent_dock_item_drag_end (di);
      event_handled = TRUE;
    }
...
static gboolean
matecomponent_dock_item_float_window_button_changed (GtkWidget *widget,
					      GdkEventButton *event,
					      gpointer data)
{
...
static gboolean
matecomponent_dock_item_button_changed (GtkWidget      *widget,
                                GdkEventButton *event)
{
  MateComponentDockItem *di;
  ...
  if (!MATECOMPONENT_DOCK_ITEM_NOT_LOCKED(widget))
    return FALSE;
...
static gboolean
matecomponent_dock_item_float_window_motion (GtkWidget *widget,
				      GdkEventMotion *event,
				      gpointer data)
{
  MateComponentDockItem *di;
...
static gboolean
matecomponent_dock_item_motion (GtkWidget      *widget,
			 GdkEventMotion *event)
{
  MateComponentDockItem *di;
...
static void
matecomponent_dock_item_add (GtkContainer *container,
		      GtkWidget    *widget)
{
  MateComponentDockItem *dock_item;
  MateComponentDockItemPrivate *priv;
  GParamSpec *pspec;

  dock_item = MATECOMPONENT_DOCK_ITEM (container);
  priv = dock_item->_priv;

  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (container));

  /*  Is this needed ? We hit this assertion when
      calling from matecomponent_dock_item_unfloat()
  */
  ...
  GTK_CONTAINER_CLASS (matecomponent_dock_item_parent_class)->add (container, widget);
...
static void
matecomponent_dock_item_set_floating (MateComponentDockItem *item, gboolean val)
{
...
static void
matecomponent_dock_item_remove (GtkContainer *container,
			 GtkWidget    *widget)
{
  MateComponentDockItem *di;
  ...
  di = MATECOMPONENT_DOCK_ITEM (container);
  ...
  GTK_CONTAINER_CLASS (matecomponent_dock_item_parent_class)->remove (container, widget);
  
}

static void
matecomponent_dock_item_forall (GtkContainer *container,
			 gboolean      include_internals,
			 GtkCallback   callback,
			 gpointer      callback_data)
{
  GtkBin *bin = (GtkBin *) container;
  MateComponentDockItem *di = (MateComponentDockItem *) container;
...
/**
 * matecomponent_dock_item_construct:
 * @new: a #MateComponentDockItem.
 * @name: Name for the new item
 * @behavior: Behavior for the new item
 *
 * Description: Constructs the @new MateComponentDockItem named @name, with the
 * specified @behavior.
 *
 * Returns: A new MateComponentDockItem widget.
 **/
void
matecomponent_dock_item_construct (MateComponentDockItem *new,
			   const gchar *name,
			   MateComponentDockItemBehavior behavior)
{
  g_return_if_fail (new != NULL);
  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (new));

  new->name = g_strdup (name);
  new->behavior = behavior;

  if (behavior & MATECOMPONENT_DOCK_ITEM_BEH_LOCKED)
    {
...
/**
 * matecomponent_dock_item_new:
 * @name: Name for the new item
 * @behavior: Behavior for the new item
 *
 * Description: Create a new MateComponentDockItem named @name, with the
 * specified @behavior.
 *
 * Returns: A new MateComponentDockItem widget.
 **/
GtkWidget *
matecomponent_dock_item_new (const gchar *name,
		      MateComponentDockItemBehavior behavior)
{
  MateComponentDockItem *new;

  new = MATECOMPONENT_DOCK_ITEM (g_object_new (matecomponent_dock_item_get_type (), NULL));

  matecomponent_dock_item_construct (new, name, behavior);

  return GTK_WIDGET (new);
}

/**
 * matecomponent_dock_item_get_child:
 * @item: A MateComponentDockItem widget
 *
 ...
 **/
GtkWidget *
matecomponent_dock_item_get_child (MateComponentDockItem *item)
{
    g_return_val_if_fail (MATECOMPONENT_IS_DOCK_ITEM (item), NULL);
...
/**
 * matecomponent_dock_item_get_name:
 * @item: A MateComponentDockItem widget.
 *
 ...
 **/
gchar *
matecomponent_dock_item_get_name (MateComponentDockItem *item)
{
 ...
 * @dock_item: A MateComponentDockItem widget
 * @type: The shadow type for @dock_item
 *
 * Description: Set the shadow type for @dock_item.
 **/
void
matecomponent_dock_item_set_shadow_type (MateComponentDockItem *dock_item,
				  GtkShadowType   type)
{
  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (dock_item));
...
/**
 * matecomponent_dock_item_get_shadow_type:
 * @dock_item: A MateComponentDockItem widget.
 *
 ...
 **/
GtkShadowType
matecomponent_dock_item_get_shadow_type (MateComponentDockItem  *dock_item)
{
  g_return_val_if_fail (dock_item != NULL, GTK_SHADOW_OUT);
  g_return_val_if_fail (MATECOMPONENT_IS_DOCK_ITEM (dock_item), GTK_SHADOW_OUT);

  return dock_item->shadow_type;
}

/**
 * matecomponent_dock_item_set_orientation:
 * @dock_item: A MateComponentDockItem widget
 ...
 **/
gboolean
matecomponent_dock_item_set_orientation (MateComponentDockItem *dock_item,
				  GtkOrientation  orientation)
{
  g_return_val_if_fail (dock_item != NULL, FALSE);
  g_return_val_if_fail (MATECOMPONENT_IS_DOCK_ITEM (dock_item), FALSE);

  if (dock_item->orientation != orientation)
    {
      if ((orientation == GTK_ORIENTATION_VERTICAL
           && (dock_item->behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_VERTICAL))
          || (orientation == GTK_ORIENTATION_HORIZONTAL
              && (dock_item->behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_HORIZONTAL)))
        return FALSE;
...
/**
 * matecomponent_dock_item_get_orientation:
 * @dock_item: A MateComponentDockItem widget.
 *
 ...
 **/
GtkOrientation
matecomponent_dock_item_get_orientation (MateComponentDockItem *dock_item)
{
  g_return_val_if_fail (MATECOMPONENT_IS_DOCK_ITEM (dock_item),
                        GTK_ORIENTATION_HORIZONTAL);
...
/**
 * matecomponent_dock_item_set_behavior:
 * @dock_item: A MateComponentDockItem widget.
 ...
 */
void
matecomponent_dock_item_set_behavior (MateComponentDockItem         *dock_item,
                               MateComponentDockItemBehavior  behavior)
{
  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (dock_item));
  ...
  if (behavior & MATECOMPONENT_DOCK_ITEM_BEH_LOCKED)
    matecomponent_dock_item_set_locked (dock_item, TRUE);

  if (behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_FLOATING &&
      dock_item->is_floating)
    matecomponent_dock_item_unfloat (dock_item);

  if (behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_VERTICAL &&
      dock_item->orientation == GTK_ORIENTATION_VERTICAL)
    matecomponent_dock_item_set_orientation (dock_item, GTK_ORIENTATION_HORIZONTAL);

  if (behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_HORIZONTAL &&
      dock_item->orientation == GTK_ORIENTATION_HORIZONTAL)
    matecomponent_dock_item_set_orientation (dock_item, GTK_ORIENTATION_VERTICAL);

  gtk_widget_queue_resize (GTK_WIDGET (dock_item));
}

/**
 * matecomponent_dock_item_get_behavior:
 * @dock_item: A MateComponentDockItem widget.
 *
...
{
  g_return_val_if_fail (MATECOMPONENT_IS_DOCK_ITEM (dock_item),
                        MATECOMPONENT_DOCK_ITEM_BEH_NORMAL);
...
void
matecomponent_dock_item_set_locked (MateComponentDockItem *dock_item,
			     gboolean        locked)
{
  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (dock_item));
  ...
  if (locked)
    {
      if (!MATECOMPONENT_DOCK_ITEM_NOT_LOCKED (dock_item))
        return;

      dock_item->behavior |= MATECOMPONENT_DOCK_ITEM_BEH_LOCKED;
...
void
matecomponent_dock_item_grab_pointer (MateComponentDockItem *item)
{
  GdkCursor *fleur;
  GdkWindow *gdk_window;

  g_assert (MATECOMPONENT_IS_DOCK_ITEM (item));
...
gboolean
matecomponent_dock_item_detach (MateComponentDockItem *item, gint x, gint y)
{
  MateComponentDockItemPrivate *priv;
  GtkWidget *widget;
  ...
  if (item->behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_FLOATING)
    return FALSE;

  item->float_x = x;
  item->float_y = y;

  matecomponent_dock_item_set_floating (item, TRUE);
...
void
matecomponent_dock_item_unfloat (MateComponentDockItem *item)
{
  MateComponentDockItemPrivate *priv;
  ...
  /* priv->child must be NULL at this point, or matecomponent_dock_item_add() barfs */
  ...
  matecomponent_dock_item_set_floating (item, FALSE);

  gtk_widget_queue_resize (GTK_WIDGET (item));
}

void
matecomponent_dock_item_attach (MateComponentDockItem *item,
			 GtkWidget *parent,
			 gint x, gint y)
{
  if (GTK_WIDGET (item)->parent != GTK_WIDGET (parent))
    {
      ...
      if (item->is_floating)
	matecomponent_dock_item_unfloat (item);
...
void
matecomponent_dock_item_drag_floating (MateComponentDockItem *item, gint x, gint y)
{
  if (item->is_floating)
    {
...
void
matecomponent_dock_item_handle_size_request (MateComponentDockItem *item,
                                     GtkRequisition *requisition)
{
  GtkBin *bin;
  GtkContainer *container;
...
void
matecomponent_dock_item_get_floating_position (MateComponentDockItem *item,
					gint *x, gint *y)
{
...
GtkWidget *
matecomponent_dock_item_get_grip (MateComponentDockItem *item)
{
  g_return_val_if_fail (MATECOMPONENT_IS_DOCK_ITEM (item), NULL);

  if (item->behavior & MATECOMPONENT_DOCK_ITEM_BEH_LOCKED)
    return NULL;
  else
