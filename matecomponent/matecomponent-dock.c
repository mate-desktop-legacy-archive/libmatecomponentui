...
G_DEFINE_TYPE (MateComponentDock, matecomponent_dock, GTK_TYPE_CONTAINER)

#define noMATECOMPONENT_DOCK_DEBUG

/* FIXME: To be removed.  */
#if defined MATECOMPONENT_DOCK_DEBUG && defined __GNUC__
#define DEBUG(x)                                        \
  do                                                    \
    {                                                   \
...
struct _MateComponentDockPrivate
{
...
static void     matecomponent_dock_size_request        (GtkWidget *widget,
                                                GtkRequisition *requisition);
static void     matecomponent_dock_size_allocate       (GtkWidget *widget,
                                                GtkAllocation *allocation);
static void     matecomponent_dock_map                 (GtkWidget *widget);
static void     matecomponent_dock_unmap               (GtkWidget *widget);
static void     matecomponent_dock_add                 (GtkContainer *container,
                                                GtkWidget *child);
static void     matecomponent_dock_remove              (GtkContainer *container,
                                                GtkWidget *widget);
static void     matecomponent_dock_forall              (GtkContainer *container,
                                                gboolean include_internals,
                                                GtkCallback callback,
                                                gpointer callback_data);
static void     matecomponent_dock_finalize            (GObject *object);
...
static void     drag_end                       (GtkWidget *widget,
                                                gpointer data);
static gboolean drag_new                       (MateComponentDock *dock,
                                                MateComponentDockItem *item,
                                                GList **area,
                                                GList *where,
                                                gint x, gint y,
                                                gboolean is_vertical);
static gboolean drag_to                        (MateComponentDock *dock,
                                                MateComponentDockItem *item,
                                                GList *where,
                                                gint x, gint y,
                                                gboolean is_vertical);
static gboolean drag_floating                  (MateComponentDock *dock,
                                                MateComponentDockItem *item,
                                                gint x, gint y,
                                                gint rel_x, gint rel_y);
static gboolean drag_check                     (MateComponentDock *dock,
                                                MateComponentDockItem *item,
                                                GList **area,
                                                gint x, gint y,
                                                gboolean is_vertical);
static void     drag_snap                      (MateComponentDock *dock,
                                                GtkWidget *widget,
                                                gint x, gint y);
...
static MateComponentDockItem *get_docked_item_by_name  (MateComponentDock *dock,
                                                const gchar *name,
                                                MateComponentDockPlacement *placement_return,
                                                guint *num_band_return,
                                                guint *band_position_return,
                                                guint *offset_return);
static MateComponentDockItem *get_floating_item_by_name (MateComponentDock *dock,
                                                 const gchar *name);

static void           connect_drag_signals      (MateComponentDock *dock,
                                                 GtkWidget *item);

...
static void
matecomponent_dock_class_init (MateComponentDockClass *class)
{
  ...
  gobject_class->finalize = matecomponent_dock_finalize;

  widget_class->size_request = matecomponent_dock_size_request;
  widget_class->size_allocate = matecomponent_dock_size_allocate;
  widget_class->map = matecomponent_dock_map;
  widget_class->unmap = matecomponent_dock_unmap;

  container_class->add = matecomponent_dock_add;
  container_class->remove = matecomponent_dock_remove;
  container_class->forall = matecomponent_dock_forall;

  dock_signals[LAYOUT_CHANGED] =
	  g_signal_new ("layout_changed",
			G_TYPE_FROM_CLASS (object_class),
			G_SIGNAL_RUN_LAST,
			G_STRUCT_OFFSET (MateComponentDockClass,
					 layout_changed),
			NULL, NULL,
...
static void
matecomponent_dock_size_request (GtkWidget *widget, GtkRequisition *requisition)
{
  MateComponentDock *dock;
  GList *lp;

  dock = MATECOMPONENT_DOCK (widget);
...
static void
matecomponent_dock_size_allocate (GtkWidget *widget, GtkAllocation *allocation)
{
  MateComponentDock *dock;
...
static void
matecomponent_dock_map (GtkWidget *widget)
{
  MateComponentDock *dock;

  g_return_if_fail (widget != NULL);
  g_return_if_fail (MATECOMPONENT_IS_DOCK(widget));

  GTK_WIDGET_CLASS(matecomponent_dock_parent_class)->map (widget);
...
static void
matecomponent_dock_unmap (GtkWidget *widget)
{
  MateComponentDock *dock;
  ...
  GTK_WIDGET_CLASS (matecomponent_dock_parent_class)->unmap (widget);
}
...
static void
matecomponent_dock_add (GtkContainer *container, GtkWidget *child)
{
  MateComponentDock *dock;

  dock = MATECOMPONENT_DOCK (container);
  matecomponent_dock_add_item (dock, MATECOMPONENT_DOCK_ITEM(child), MATECOMPONENT_DOCK_TOP, 0, 0, 0, TRUE);
}

static gboolean
remove_from_band_list (GList **list, MateComponentDockBand *child)
{
  GList *lp;
...
static void
matecomponent_dock_remove (GtkContainer *container, GtkWidget *widget)
{
  MateComponentDock *dock;
...
static void
matecomponent_dock_forall (GtkContainer *container,
                   gboolean include_internals,
                   GtkCallback callback,
                   gpointer callback_data)
{
  ...
  g_return_if_fail (MATECOMPONENT_IS_DOCK (container));
...
static void
matecomponent_dock_finalize (GObject *object)
{
  MateComponentDock *self = MATECOMPONENT_DOCK (object);

  g_free (self->_priv);
  self->_priv = NULL;

  G_OBJECT_CLASS (matecomponent_dock_parent_class)->finalize (object);
...
static void
new_band_setup (MateComponentDock    *dock,
		GtkWidget     *new_band,
		GtkOrientation orientation)
{
      matecomponent_dock_band_set_orientation (
	      MATECOMPONENT_DOCK_BAND (new_band), orientation);
...
/* Case (I): Dock `item' into a new band next to `where' in the
   ...
   first one in `area'.  */
static gboolean
drag_new (MateComponentDock *dock,
          MateComponentDockItem *item,
          GList **area,
          GList *where,
          gint x, gint y,
          gboolean is_vertical)
{
  MateComponentDockBand *new_band;
  GList *next;
  ...
  if (next != NULL)
    {
      ...
      band = MATECOMPONENT_DOCK_BAND (next->data);

      num_children = matecomponent_dock_band_get_num_children (band);

      if (num_children == 0
          || (num_children == 1
              && GTK_WIDGET (band) == GTK_WIDGET (item)->parent))
        new_band = MATECOMPONENT_DOCK_BAND (next->data);
    }
  ...
  if (new_band == NULL)
    {
      new_band = MATECOMPONENT_DOCK_BAND (matecomponent_dock_band_new ());
  ...
  matecomponent_dock_item_attach (item, GTK_WIDGET (new_band), x, y);

  /* Prepare the band for dragging of `item'.  */
  matecomponent_dock_band_drag_begin (new_band, item);

  /* Set the offset of `item' in the band.  */
  if (is_vertical)
    matecomponent_dock_band_set_child_offset (new_band, GTK_WIDGET (item),
...
static gboolean
drag_to (MateComponentDock *dock,
         MateComponentDockItem *item,
         GList *where,
         gint x, gint y,
         gboolean is_vertical)
{
  ...
  return matecomponent_dock_band_drag_to (MATECOMPONENT_DOCK_BAND (where->data), item, x, y);
}

/* Case (III): Move a floating (i.e. floating) item.  */
static gboolean
drag_floating (MateComponentDock *dock,
               MateComponentDockItem *item,
               gint x,
               gint y,
               gint rel_x,
  ...
  if (!item->is_floating && item_widget->parent != dock_widget)
    {
	  ...
	  matecomponent_dock_item_detach (item, x, y);
          if (item->in_drag)
  ...
  else
    {
      ...
      matecomponent_dock_item_drag_floating (item, x, y);
    }
...
/* Check if `item' can be docked to any of the DockBands of the dock
   ...
   FALSE.  */
static gboolean
drag_check (MateComponentDock *dock,
            MateComponentDockItem *item,
            GList **area,
            gint x, gint y,
            gboolean is_vertical)
  ...
  for (lp = *area; lp != NULL; lp = lp->next)
    {
      ...
      band = MATECOMPONENT_DOCK_BAND (lp->data);
...
/* Snap the MateComponentDockItem `widget' to `dock' at the specified
   position.  */
static void
drag_snap (MateComponentDock *dock,
           GtkWidget *widget,
           gint x, gint y)
{
#define SNAP 50
  MateComponentDockItem *item;
  MateComponentDockItemBehavior item_behavior;
  gint win_x, win_y;
  gint rel_x, rel_y;
  gboolean item_allows_horizontal, item_allows_vertical;

  item = MATECOMPONENT_DOCK_ITEM (widget);

  item_behavior = matecomponent_dock_item_get_behavior (item);
  item_allows_horizontal = ! (item_behavior
                              & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_HORIZONTAL);
  ...
  if (dock->floating_items_allowed
      && ! (item_behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_DETACH))
    drag_floating (dock, item, x, y, rel_x, rel_y);

  /* If still not floating, fall back to moving the item in its own
     band.  */
  if (! item->is_floating)
    matecomponent_dock_band_drag_to (MATECOMPONENT_DOCK_BAND (GTK_WIDGET (item)->parent),
                             item, rel_x, rel_y);
}
...
drag_begin (GtkWidget *widget, gpointer data)
{
  ...
  dock = MATECOMPONENT_DOCK (data);
  item = MATECOMPONENT_DOCK_ITEM (widget);

  /* Communicate all the bands that `widget' is currently being
     dragged.  */
  g_list_foreach (dock->top_bands, (GFunc) matecomponent_dock_band_drag_begin, item);
  g_list_foreach (dock->bottom_bands, (GFunc) matecomponent_dock_band_drag_begin, item);
  g_list_foreach (dock->right_bands, (GFunc) matecomponent_dock_band_drag_begin, item);
  g_list_foreach (dock->left_bands, (GFunc) matecomponent_dock_band_drag_begin, item);
}
...
static void
drag_end_bands (GList **list, MateComponentDockItem *item)
{
  GList *lp;
  MateComponentDockBand *band;
  ...
  while (lp != NULL)
    {
      band = MATECOMPONENT_DOCK_BAND(lp->data);
      matecomponent_dock_band_drag_end (band, item);

      if (matecomponent_dock_band_get_num_children (band) == 0)
        {
          GList *next;
...
/* Handle a drag motion on the MateComponentDockItem `widget'.  This is
   connected to the "drag_motion" of all the children being added to
   the MateComponentDock, and tries to dock the dragged item at the current
...
drag_motion (GtkWidget *widget,
             ...
             gpointer data)
{
  drag_snap (MATECOMPONENT_DOCK (data), widget, x, y);
}



static MateComponentDockItem *
get_docked_item_by_name (MateComponentDock *dock,
                         const gchar *name,
                         MateComponentDockPlacement *placement_return,
                         guint *num_band_return,
                         guint *band_position_return,
...
{
  {
    ...
    areas[] =
    {
      { NULL, MATECOMPONENT_DOCK_TOP },
      { NULL, MATECOMPONENT_DOCK_BOTTOM },
      { NULL, MATECOMPONENT_DOCK_LEFT },
      { NULL, MATECOMPONENT_DOCK_RIGHT },
      { NULL, MATECOMPONENT_DOCK_FLOATING }
    };
    GList *lp;
    guint i;
    ...
    for (i = 0; i < 4; i++)
      {
        ...
        for (lp = areas[i].band_list, num_band = 0;
             ...
             lp = lp->next, num_band++)
          {
            ...
            band = MATECOMPONENT_DOCK_BAND(lp->data);
            item = matecomponent_dock_band_get_item_by_name (band,
                                                     name,
...
static MateComponentDockItem *
get_floating_item_by_name (MateComponentDock *dock,
                           const gchar *name)
{
  GList *lp;
  MateComponentDockItem *item;
...
static void
connect_drag_signals (MateComponentDock *dock,
                      GtkWidget *item)
{
...
/**
 * matecomponent_dock_new:
 *
 * Description: Creates a new #MateComponentDock widget.
 *
 * Return value: The new widget.
 **/
GtkWidget *
matecomponent_dock_new (void)
{
  MateComponentDock *dock;
  GtkWidget *widget;

  dock = g_object_new (matecomponent_dock_get_type (), NULL);
...
/**
 * matecomponent_dock_allow_floating_items:
 * @dock: A pointer to a #MateComponentDock widget
 ...
 **/
void
matecomponent_dock_allow_floating_items (MateComponentDock *dock,
                                 gboolean enable)
{
...
static GList **
get_band_list (MateComponentDock *dock, MateComponentDockPlacement placement)
{
  GList **band_ptr = NULL;

  switch (placement)
    {
    case MATECOMPONENT_DOCK_TOP:
      band_ptr = &dock->top_bands;
      break;
    case MATECOMPONENT_DOCK_BOTTOM:
      band_ptr = &dock->bottom_bands;
      break;
    case MATECOMPONENT_DOCK_LEFT:
      band_ptr = &dock->left_bands;
      break;
    case MATECOMPONENT_DOCK_RIGHT:
 ...
 * %MATECOMPONENT_DOCK_TOP, %MATECOMPONENT_DOCK_RIGHT, %MATECOMPONENT_DOCK_BOTTOM or
 * %MATECOMPONENT_DOCK_LEFT, and specifies what area of the dock should
 ...
 **/
void
matecomponent_dock_add_item (MateComponentDock *dock,
		      MateComponentDockItem *item,
		      MateComponentDockPlacement placement,
		      guint band_num,
		      gint position,
		      ...
		      gboolean in_new_band)
{
  ...
  if (placement == MATECOMPONENT_DOCK_FLOATING)
    {
      g_warning ("Floating dock items not supported by `matecomponent_dock_add_item'.");
      return;
    }
  ...
  if (in_new_band || p == NULL)
    {
      ...
      new_band = matecomponent_dock_band_new ();
      ...
      if (placement == MATECOMPONENT_DOCK_TOP || placement == MATECOMPONENT_DOCK_BOTTOM)
        matecomponent_dock_band_set_orientation (MATECOMPONENT_DOCK_BAND (new_band),
                                         GTK_ORIENTATION_HORIZONTAL);
      else
        matecomponent_dock_band_set_orientation (MATECOMPONENT_DOCK_BAND (new_band),
  ...
  band = MATECOMPONENT_DOCK_BAND (p->data);
  matecomponent_dock_band_insert (band, GTK_WIDGET(item), offset, position);
...
/**
 * matecomponent_dock_add_floating_item:
 * @dock: A #MateComponentDock widget
 ...
 **/
void
matecomponent_dock_add_floating_item (MateComponentDock *dock,
                              MateComponentDockItem *item,
                              gint x, gint y,
                              GtkOrientation orientation)
{
  ...
  g_return_if_fail (MATECOMPONENT_IS_DOCK_ITEM (item));

  matecomponent_dock_item_set_orientation (item, orientation);
...
/**
 * matecomponent_dock_set_client_area:
 * @dock: A #MateComponentDock widget
 ...
 **/
void
matecomponent_dock_set_client_area (MateComponentDock *dock, GtkWidget *widget)
{
...
/**
 * matecomponent_dock_get_client_area:
 * @dock: A #MateComponentDock widget.
 *
 ...
 **/
GtkWidget *
matecomponent_dock_get_client_area (MateComponentDock *dock)
{
  return dock->client_area;
}

/**
 * matecomponent_dock_get_item_by_name:
 * @dock: A #MateComponentDock widget.
 ...
 * the placement is %MATECOMPONENT_DOCK_FLOATING *@num_band_return,
 * *@band_position_return and *@offset_return are not set.
 *
 * Returns: The named #MateComponentDockItem widget, or %NULL if no item with
 * such name exists.
 **/
MateComponentDockItem *
matecomponent_dock_get_item_by_name (MateComponentDock *dock,
                             const gchar *name,
                             MateComponentDockPlacement *placement_return,
                             guint *num_band_return,
                             guint *band_position_return,
                             guint *offset_return)
{
  ...
  if (item != NULL)
    {
      if (placement_return != NULL)
        *placement_return = MATECOMPONENT_DOCK_FLOATING;
      return item;
    }
...
static void
layout_add_floating (MateComponentDock *dock,
  ...
  for (lp = dock->floating_children; lp != NULL; lp = lp->next)
    {
      ...
      item = MATECOMPONENT_DOCK_ITEM (lp->data);

      orientation = matecomponent_dock_item_get_orientation (item);
      matecomponent_dock_item_get_floating_position (item, &x, &y);

      matecomponent_dock_layout_add_floating_item (layout, item,
                                           x, y,
                                           orientation);
    }
}
...
static void
layout_add_bands (MateComponentDock *dock,
                  MateComponentDockLayout *layout,
                  MateComponentDockPlacement placement,
                  GList *band_list)
{
  ...
  for (lp = band_list, band_num = 0;
       ...
       lp = lp->next, band_num++)
    {
      ...
      band = MATECOMPONENT_DOCK_BAND(lp->data);
      matecomponent_dock_band_layout_add (band, layout, placement, band_num);
    }
}

/**
 * matecomponent_dock_get_layout:
 * @dock: A #MateComponentDock widget
 *
 * Description: Retrieve the layout of @dock.
 *
 * Returns: @dock's layout as a #MateComponentDockLayout object.
 **/
MateComponentDockLayout *
matecomponent_dock_get_layout (MateComponentDock *dock)
{
  MateComponentDockLayout *layout;

  layout = matecomponent_dock_layout_new ();

  layout_add_bands (dock, layout, MATECOMPONENT_DOCK_TOP, dock->top_bands);
  layout_add_bands (dock, layout, MATECOMPONENT_DOCK_BOTTOM, dock->bottom_bands);
  layout_add_bands (dock, layout, MATECOMPONENT_DOCK_LEFT, dock->left_bands);
  layout_add_bands (dock, layout, MATECOMPONENT_DOCK_RIGHT, dock->right_bands);
...
/**
 * matecomponent_dock_add_from_layout:
 * @dock: The #MateComponentDock widget
 ...
 **/
gboolean
matecomponent_dock_add_from_layout (MateComponentDock       *dock,
			     MateComponentDockLayout *layout)
{
  return matecomponent_dock_layout_add_to_dock (layout, dock);
}

static GList **
find_band_list (MateComponentDock          *dock,
		MateComponentDockBand      *band,
		MateComponentDockPlacement *placement)
{
...
static gboolean
insert_into_band_list (MateComponentDock     *dock,
		       GList         **band_list,
		       GtkOrientation  orientation,
		       MateComponentDockItem *item,
		       gboolean        prepend)
{
  ...
  if (item->behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_VERTICAL)
    orientation = GTK_ORIENTATION_HORIZONTAL;

  if (item->behavior & MATECOMPONENT_DOCK_ITEM_BEH_NEVER_HORIZONTAL)
    orientation = GTK_ORIENTATION_VERTICAL;

  if (!matecomponent_dock_band_append (
	  MATECOMPONENT_DOCK_BAND (new_band), GTK_WIDGET (item), 0))
    return FALSE;
...
gint
_matecomponent_dock_handle_key_nav (MateComponentDock     *dock,
			     MateComponentDockBand *band,
			     MateComponentDockItem *item,
			     GdkEventKey    *event)
{
  ...
  if (placement == MATECOMPONENT_DOCK_LEFT ||
      placement == MATECOMPONENT_DOCK_RIGHT)
    {
  ...
  if (cross_band_dir == -1)
    {
      for (entry = entry->prev; !was_inserted && entry;
	   entry = entry->prev)
        was_inserted = matecomponent_dock_band_append (
		entry->data, GTK_WIDGET (item), 0);

      if (!was_inserted &&
	  ((*band_list)->data != band ||
	   matecomponent_dock_band_get_num_children (band) > 0))
        {
      ...
      if (!was_inserted)
        {
	  if (placement == MATECOMPONENT_DOCK_BOTTOM)
	    {
	      was_inserted = insert_into_band_list (
		      dock, &dock->top_bands, orientation, item, FALSE);
	    }
	  else if (placement == MATECOMPONENT_DOCK_RIGHT)
	    {
	      was_inserted = insert_into_band_list (
  ...
  if (cross_band_dir == +1)
    {
      ...
      if (!was_inserted)
        {
	  if (placement == MATECOMPONENT_DOCK_TOP)
	    {
	      was_inserted = insert_into_band_list (
		      dock, &dock->bottom_bands, orientation, item, TRUE);
	    }
	  else if (placement == MATECOMPONENT_DOCK_LEFT)
	    {
	      was_inserted = insert_into_band_list (
  ...
  if (!was_inserted)
    { /* geometry issue */
      if (!matecomponent_dock_band_append (band, GTK_WIDGET (item), 0))
